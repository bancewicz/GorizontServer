...
typedef struct
{
  ...
  byte YType;
  // 24+2=26 байта
  byte reserve1[200-26];
  SChl Ch[32];  // 32*200
  byte reserve2[49*200-32*200];
  void Correct();
} SSetup;
//---------------------------------------------------------------------------
int LineSeriesSel=0;TLineSeries* LSrs[32];
SSetup Stp;
#define MinTime 40268 // 31.03.2010
//---------------------------------------------------------------------
SStpChart SChart;SStpChart StpChart;
TColor MaskColor=(TColor)RGB(238,239,240),ColorChange=(TColor)RGB(239,239,240);
//---------------------------------------------------------------------------
__fastcall TFChartDlg::TFChartDlg(TComponent* AOwner)
	: TForm(AOwner)
{
  for(int i=0;i<32;i++) LSrs[i]=(TLineSeries*) FMain->Chart->Series[i];
  AnsiString s;
  for(int i=0;i<32;i++) { s=" ";s+=(i/2+1);if((i+1)&1) s+="A";else s+="B";LSrs[i]->Title=s;}
...
void __fastcall TFChartDlg::CBChOn1Click(TObject *Sender)
{
  ...
  {
    LSrs[CBCh1->ItemIndex]->Active=CBChOn1->Checked;
    // ChartDlg
    //FMain->PointCount();
    // ChartDlg
...
void __fastcall TFChartDlg::CBChOn2Click(TObject *Sender)
{
  ...
  {
    LSrs[CBCh2->ItemIndex]->Active=CBChOn2->Checked;
    // ChartDlg
    //FMain->PointCount();
    // ChartDlg
    //FMain->DrawGrid->Repaint();
