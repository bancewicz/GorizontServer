...
typedef  struct {
  ...
  WORD   tuningmode:1;  // Режим юстировки
  WORD   reserve1:1;    // Служебные
  WORD   reserve2:1;    // Служебные
  WORD   reserve3:1;    // Служебные

  WORD   reserve4:1;    // Служебные
  WORD   reserve5:1;    // Служебные
  WORD   bufferoff:1;   // Отключить кольцевой буфер
  WORD   reserve7:1;    // Служебные

  WORD   reserve8:1;    // Служебные
  WORD   reserve9:1;    // Служебные
  WORD   reserve10:1;   // Служебные
  WORD   reserve11:1;   // Служебные

  WORD   reserve12:1;   // Служебные
  WORD   reserve13:1;   // Служебные
  WORD   reserve14:1;   // Служебные
  WORD   reserve15:1;   // Служебные
...
//Снимаем данные по режиму работы
MODE=(*(WORK_MODE_AND3*)b);

//Вычисляем параметры режима работы

TUNNG_MODE=MODE&(1<<0); //Режим юстировки - отключена фильтрация        бит 0 режимаработы
BUFFER_OFF=MODE&(1<<6); //Кольцевойбуфер отключен                       бит 6 режимаработы
